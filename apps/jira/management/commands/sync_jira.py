...
from apps.jira.services import sync_jira_organization, sync_all_jira_organizations


class Command(BaseCommand):
    help = 'Sync data from JIRA API'
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '--test-connection',
            action='store_true',
            help='Test connection to JIRA organizations',
        )
    ...
    def handle(self, *args, **options):
        ...
        if org_name:
            ...
            try:
                org = JiraOrganization.objects.get(name__icontains=org_name)
                ...
                sync_log = sync_jira_organization(org.id)
            ...
            sync_logs = sync_all_jira_organizations()
            ...
            for sync_log in sync_logs:
                if sync_log.status == 'success':
                    ...
                    self.stdout.write(
                        self.style.SUCCESS(
                            f'✓ {sync_log.jira_organization.name}: '
                    ...
                    self.stdout.write(
                        self.style.ERROR(
                            f'✗ {sync_log.jira_organization.name}: {sync_log.error_message}'
                        )
                    )
    ...
    def _test_connections(self, org_name=None):
        ...
        if org_name:
            try:
                orgs = [JiraOrganization.objects.get(name__icontains=org_name)]
            except JiraOrganization.DoesNotExist:
                raise CommandError(f'Organization matching "{org_name}" does not exist')
        else:
            orgs = JiraOrganization.objects.all()

        self.stdout.write('Testing JIRA connections...\n')
